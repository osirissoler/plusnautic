...
export default function CheckoutScreen({ navigation, route }: any) {
			...
			checkStorage('USER_PHARMACY', (response: any) => {
				checkStorage('USER_LOGGED', async (user: any) => {
					checkStorage('TOKEN', async (resp: any) => {
						setTOKEN('ExponentPushToken[yktQf7OoUbbgeI5OGZH25b]'//resp
							)
						//console.log(user); 
						userId = user
	...
	const validateFields = () => {
		...
		checkStorage('USER_PHARMACY', (response: any) => {
			...
			checkStorage('USER_LOGGED', (userId: any) => {
				...
				const clientBankFeeAux = (TotalOrder*0.03)+0.23//route.params.bankFee;
				 const clientBankFee_state_tax = clientBankFeeAux * 0.105;
				const clientBankFee_municipal_tax = clientBankFeeAux * 0.01;
				const clientBankFee= clientBankFeeAux
				//clientBankFee_state_tax+clientBankFee_municipal_tax
	...
	const payWithStripe = (order: any, pharmacy: any) => {
		...
		//	createOrderNotification(order, pharmacy);
	...
	const createOrderNotification = (order: any, pharmacy: any) => {
		// console.log("create notification", order, pharmacy)
		// checkStorage('TOKEN', async (response: any) => {
		// 	setTOKEN(response)
		...
		// 		order_status: 1
		// 	});
		// const dbRefPush = await firebase.firestore().collection('notificationsPush').add({message: 'A new order #' + order.code + ' has been placed.',
		// 	seen: false,
		...
		// 	token: response
		// };
		// dbRefPush.add(notificationPush);
		//console.log("notification created",dbRef, dbRefPush)
	...
	return (
		<SafeAreaView style={styles.container}>
			...
			{
			!(!placeToPayOperationFineshed && showPlaceToPayview/*&&!showLoading*/)  && <View style={styles.body}>
				<View>
