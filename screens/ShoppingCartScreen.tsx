...
export default function ShoppingCartScreen({ navigation }: any) {
	...
	const modifyPrice = (type: number, product: any) => {
		...
		const data = { pharmacy_id: pharmacy['id'], id: product.pharmacy_product_id };
	...
	const calculatePrices = (products: any) => {
		let prices = {
			...
			fee_ivu_est: 0.0,
			fee_ivu_mun: 0.0,
			delivery_ivu_est: 0.0,
			delivery_ivu_mun: 0.0,
			fee: 1.0,
			deliveryFee: 6.0,
			total: 0.0,
			 bankFee: 0.0,
		...
		prices.ivu_est =prices.subTotal * StateTax;
		prices.ivu_mun = prices.subTotal * MunicipalTax;
		prices.fee_ivu_est = prices.fee*StateTax;
		prices.fee_ivu_mun =prices.fee* MunicipalTax;
		prices.delivery_ivu_est = prices.deliveryFee *StateTax;
		prices.delivery_ivu_mun = prices.deliveryFee * MunicipalTax;
		prices.total = roundNumber(prices.subTotal + prices.ivu_est + prices.ivu_mun+ prices.fee 
			+ prices.fee_ivu_est +prices.fee_ivu_mun
			+ prices.deliveryFee +prices.delivery_ivu_est
			+ prices.delivery_ivu_mun)
		 prices.bankFee = (prices.total*0.03)+0.23+(prices.bankFee*MunicipalTax)
		// prices.bankFee_ivu_est=prices.bankFee*StateTax
		// prices.bankFee_ivu_mun=prices.bankFee*MunicipalTax
		// prices.total = roundNumber(
		// 	prices.subTotal + prices.ivu_est + prices.ivu_mun
		// 	+ prices.fee + prices.fee_ivu_est +prices.fee_ivu_mun
		// 	+ prices.deliveryFee +prices.delivery_ivu_est+ prices.delivery_ivu_mun
		// 	//+ prices.bankFee+prices.bankFee_ivu_est+prices.bankFee_ivu_mun
		// 	);
...
function ProductCard({ item, pharmacy, deleteProduct, modifyPrice, fetchProducts }: any) {
	...
	const fetchProductData = () => {
		const url = '/products/getPharmaciesProductByid';
		const data = { pharmacy_id: pharmacy['id'], id: item.pharmacy_product_id };
		sendData(url, data).then((response) => {
